...
bool can_disarm_trap_type(int traptype)
{
	switch (traptype) {
		...
		case TRAP_OF_UNKNOWN_DOUBLE:
		case TRAP_OF_UNKNOWN_TRIPLE:
		case TRAP_OF_UNKNOWN_QUAD:
		case TRAP_OF_UNKNOWN_PENTA:
		case TRAP_NASTY1:
		case TRAP_NASTY2:
		case TRAP_NASTY3:
		case TRAP_NASTY4:
...
bool player_activate_trap_type(s16b y, s16b x, object_type *i_ptr, s16b item)
{
	...
	if (trap == TRAP_OF_UNKNOWN_DOUBLE) {
	...
	if (trap == TRAP_OF_UNKNOWN_TRIPLE) {
		if (c_ptr->info & (CAVE_TRDT)) {
			t_info[trap].ident = TRUE;
		}
	...
	case TRAP_OF_STIR_II:
		{
			ident = TRUE;
			msg_print("You hear a loud stirring sound.");
			for (k = 0; k < 20; k++)
			{
				(void)alloc_monster(MAX_SIGHT + 5, FALSE);
			}
			...
			if (randint(2) == 1) {
				...
				if (ident) msg_print("You identified that trap as Loud Stir Trap.");
				ident = FALSE;
	...
	case TRAP_OF_STIR_III:
		{
			ident = TRUE;
			msg_print("You hear a very loud stirring sound.");
			for (k = 0; k < 40; k++)
			{
				(void)alloc_monster(MAX_SIGHT + 5, FALSE);
			}
			...
			if (randint(2) == 1) {
				...
				if (ident) msg_print("You identified that trap as Droning Stir Trap.");
				ident = FALSE;
	...
	case TRAP_OF_STIR_IV:
		{
			ident = TRUE;
			msg_print("You hear an extremely loud stirring sound.");
			for (k = 0; k < 100; k++)
			{
				(void)alloc_monster(MAX_SIGHT + 5, FALSE);
			}
			...
			if (randint(5) != 1) {
				...
				if (ident) msg_print("You identified that trap as Deafening Stir Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_SQUAD:
		...
		/* Summon Squad Trap */
	case TRAP_OF_SUMMON_SQUAD_X:
		{
			msg_print("A preposterous spell hangs in the air.");
			for (k = 0; k < (10 + randint(10) + randint(20)); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type], 0);
			}
			...
			if (randint(2) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Mega Squad Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_BAT_X:
		...
		/* Summon Bird Trap */
	case TRAP_OF_SUMMON_BIRD:
		{
			msg_print("A winged spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_BIRD);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Bird Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_TREASURE:
		...
		/* Summon Treasure Trap */
	case TRAP_OF_SUMMON_TREASURE_X:
		{
			msg_print("A clinging spell hangs in the air.");
			for (k = 0; k < (3 + randint(3)); k++)
			{
	...
	case TRAP_OF_SUMMON_THUNDERLORD:
		...
		/* Summon Thunderlord Trap */
	case TRAP_OF_SUMMON_THUNDERLORD_X:
		{
			msg_print("A majestic spell hangs in the air.");
			for (k = 0; k < (3 + randint(3)); k++)
			{
	...
	case TRAP_OF_SUMMON_ELEMENTAL:
		...
		/* Summon Elemental Trap */
	case TRAP_OF_SUMMON_ELEMENTAL_X:
		{
			msg_print("An unstoppable spell hangs in the air.");
			for (k = 0; k < (3 + randint(3)); k++)
			{
	...
	case TRAP_OF_SUMMON_CAT_X:
		...
		/* Summon Dog Trap */
	case TRAP_OF_SUMMON_DOG:
		{
			msg_print("A barking spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_DOG);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Dog Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_DOG_X:
		...
		/* Summon Insect Trap */
	case TRAP_OF_SUMMON_INSECT:
		{
			msg_print("A crawly spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_INSECT);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Insect Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_INSECT_X:
		...
		/* Summon Ogre Trap */
	case TRAP_OF_SUMMON_OGRE:
		{
			msg_print("A gutteral spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_OGRE);
			...
			if (randint(5) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Ogre Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_LIZARD:
		{
			msg_print("A forked spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_LIZARD);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Lizard Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_LIZARD_X:
		...
		/* Summon Hulk Trap */
	case TRAP_OF_SUMMON_HULK:
		{
			msg_print("A huge spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_HULK);
			...
			if (randint(3) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Hulk Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_HULK_X:
		...
		/* Summon Monkey Trap */
	case TRAP_OF_SUMMON_MONKEY:
		{
			msg_print("A wild spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_MONKEY);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Monkey Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_MONKEY_X:
		...
		/* Summon Eye Trap */
	case TRAP_OF_SUMMON_EYE:
		{
			msg_print("A visible spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_EYE);
			...
			if (randint(5) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Eye Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_EYE_X:
		...
		/* Summon Person Trap */
	case TRAP_OF_SUMMON_PERSON:
		{
			msg_print("A talking spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_PERSON);
			...
			if (randint(4) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Person Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_PERSON_X:
		...
		/* Summon Jelly Trap */
	case TRAP_OF_SUMMON_JELLY:
		{
			msg_print("A glibbery spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_JELLY);
			...
			if (randint(5) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Jelly Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_JELLY_X:
		...
		/* Summon Kobold Trap */
	case TRAP_OF_SUMMON_KOBOLD:
		{
			msg_print("A meek spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_KOBOLD);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Kobold Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_KOBOLD_X:
		...
		/* Summon Quadruped Trap */
	case TRAP_OF_SUMMON_QUADRUPED:
		{
			msg_print("A dusty spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_QUADRUPED);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Quadruped Trap.");
				ident = FALSE;
		...
		/* Summon Quadruped Trap */
	case TRAP_OF_SUMMON_QUADRUPED_X:
		{
			msg_print("A dusty spell hangs in the air.");
			for (k = 0; k < (3 + randint(3)); k++)
			{
		...
		/* Summon Yeek Trap */
	case TRAP_OF_SUMMON_YEEK:
		{
			msg_print("A squeaking spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_YEEK);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Yeek Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_SEXY_GIRL_X:
		{
			msg_print("A feminine spell hangs in the air.");
			for (k = 0; k < 2; k++)
			{
	...
	case TRAP_OF_SUMMON_BEETLE_X:
		...
		/* Summon Golem Trap */
	case TRAP_OF_SUMMON_GOLEM:
		{
			msg_print("A mechanic spell hangs in the air.");
			for (k = 0; k < randint(3); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type],
				                         SUMMON_GOLEM);
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Golem Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_SHORE_X:
		...
		/* Summon Aqua Trap - requires terrain where aquatic monsters can exist --Amy */
	case TRAP_OF_SUMMON_AQUA:
		{
			msg_print("Water floods the dungeon!");

			fill_area_terrain(p_ptr->py, p_ptr->px, 3, FEAT_SHAL_WATER, 100);

			msg_print("A rushing spell hangs in the air.");
			...
			if (randint(2) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Aqua Trap.");
				ident = FALSE;
	...
	case TRAP_OF_SUMMON_MOUNTAIN:
		...
		/* Summon Mountain Trap */
	case TRAP_OF_SUMMON_MOUNTAIN_X:
		{
			msg_print("A cold spell hangs in the air.");
			for (k = 0; k < (3 + randint(3)); k++)
			{
	...
	case TRAP_OF_SUMMON_GREATER_UNDEAD:
		...
		/* Summon Greater Undead Trap */
	case TRAP_OF_SUMMON_GREATER_UNDEAD_X:
		{
			msg_print("An old and evil spell hangs in the air.");
			for (k = 0; k < (3 + randint(3)); k++)
			{
	...
	case TRAP_OF_SUMMON_GREATER_DEMON:
		...
		/* Summon Greater Demon Trap */
	case TRAP_OF_SUMMON_GREATER_DEMON_X:
		{
			msg_print("An old and evil spell hangs in the air.");
			for (k = 0; k < (3 + randint(3)); k++)
			{
	...
	case TRAP_OF_SUMMON_GREATER_DRAGON:
		...
		/* Summon Greater Dragon Trap */
	case TRAP_OF_SUMMON_GREATER_DRAGON_X:
		{
			msg_print("An old and evil spell hangs in the air.");
			for (k = 0; k < (3 + randint(3)); k++)
			{
	...
	case TRAP_OF_INTERLACE_LOW:
		{
			if (!p_ptr->nastytrap3) t_info[trap].ident = TRUE;

			int trapcount = 5 + randint(5);
			...
			ident = FALSE;
			if (!p_ptr->nastytrap3) msg_print("You identified that trap as Distribution Trap.");
			break;
		}
	...
	case TRAP_OF_INTERLACE_HI:
		{
			if (!p_ptr->nastytrap3) t_info[trap].ident = TRUE;

			int trapcount = 50 + randint(50);
			...
			ident = FALSE;
			if (!p_ptr->nastytrap3) msg_print("You identified that trap as Wide Scatter Trap.");
			break;
		}
	...
	case TRAP_OF_STEAL_ITEM_III:
		...
		/* Summon Quylthulg Trap */
	case TRAP_OF_SUMMON_QUYLTHULG:
		{
			for (k = 0; k < randint(3); k++)
			{
			...
			if (randint(10) == 1) {
				...
				if (ident) msg_print("You identified that trap as Summon Quylthulg Trap.");
				ident = FALSE;
		...
		/* Summon Quylthulg Trap */
	case TRAP_OF_SUMMON_QUYLTHULG_X:
		{
			for (k = 0; k < (3 + randint(3)); k++)
			{
				ident |= summon_specific(y, x, max_dlv_real[dungeon_type], SUMMON_QUYLTHULG);
	...
	/* trap that removes "junk" (stuff that can be made into arrows); apparently both TV_JUNK and TV_SKELETON --Amy */
	case TRAP_OF_UNTRASH:
		{
			object_type *j_ptr;
			s16b j;
			...
			for (j = 0; j < INVEN_WIELD; j++)
			{
				...
				if (j_ptr->tval == TV_JUNK)
				{
					...
					if (!ident)
					{
						msg_print("The trashman comes and clears unnecessary stuff from your inventory.");
					}
					else
					{
						msg_print("Your trash has been taken out.");
					}
					ident = TRUE;
				}
			}
			...
			if (!ident) msg_print("The trashman looks at you, determines that you have no trash, and leaves.");
			break;
		}
	...
	case TRAP_OF_BOTTLE_SNATCH:
		{
			...
			for (j = 0; j < INVEN_WIELD; j++)
			{
				...
				if (j_ptr->tval == TV_BOTTLE)
				{
					...
					if (!ident)
					{
						msg_print("A malicious hand filches a bunch of bottles from your pack.");
					}
					else
					{
						msg_print("You're out of bottles.");
					}
					ident = TRUE;
				}
			}
	...
	case TRAP_OF_GROUP_II:
		{
			...
			/* Never known */
			ident = FALSE;
			if (!p_ptr->nastytrap3) msg_print("You identified that trap as Multiple Group Trap.");

		break;
		}

	case TRAP_OF_GROUP_III:
		{
			if (!p_ptr->nastytrap3) t_info[trap].ident = TRUE;
			...
			/* Never known */
			ident = FALSE;
			if (!p_ptr->nastytrap3) msg_print("You identified that trap as Many Group Trap.");

		break;
		}

	case TRAP_OF_GROUP_IV:
		{
			if (!p_ptr->nastytrap3) t_info[trap].ident = TRUE;
			...
			/* Never known */
			ident = FALSE;
			if (!p_ptr->nastytrap3) msg_print("You identified that trap as Plentiful Group Trap.");
	...
	case TRAP_OF_STEAL_EQUIPMENT_III:
		...
		/* Trap of Blast Armor, by Amy: equipped stuff with positive base AC becomes zero */
	case TRAP_OF_BLAST_ARMOR:
		{
			s16b j;
			bool message = FALSE;
			object_type *j_ptr;
	...
	case TRAP_OF_INVERT_WEAPON_III:
		...
		/* Trap of Blast Weapon, by Amy: weapon dice become zero */
	case TRAP_OF_BLAST_WEAPON:
		{
			s16b j;
			bool message = FALSE;
			object_type *j_ptr;
	...
	case TRAP_OF_ANIMATION_GOLEM:
		{
			ident = TRUE;
			msg_print("The statues come to life!");
			...
			for (nx = x - 10; nx <= x + 10; nx++)
				for (ny = y - 10; ny <= y + 10; ny++)
				{
					...
					if (rand_int(distance(ny, nx, y, x)) > 6)
					{
						summon_specific(ny, nx, max_dlv_real[dungeon_type], SUMMON_GOLEM);
					}
				}
	...
	case TRAP_OF_ANIMATION_X:
		{
			...
			for (nx = x - 12; nx <= x + 12; nx++)
				for (ny = y - 12; ny <= y + 12; ny++)
				{
					if (!in_bounds (ny, nx)) continue;
			...
			if (randint(2) == 1) {
				...
				if (ident) msg_print("You identified that trap as Hyper Animation Trap.");
				ident = FALSE;
	...
	case TRAP_OF_TERRAIN_DEAD_MARSH:
		ident = TRUE;
		fill_area_terrain(p_ptr->py, p_ptr->px, 6, FEAT_DEAD_MARSH, 40);

		msg_print("Marshland seems to fill the dungeon.");
		...
		if (randint(5) == 1) {
			...
			msg_print("You identified that trap as Dead Marsh Trap.");
		}
		break;

	case TRAP_OF_TERRAIN_BEES:
		ident = TRUE;
		fill_area_terrain(p_ptr->py, p_ptr->px, 5, FEAT_BEEHIVE, 10);

		msg_print("A bunch of beehives plop out of thin air!");
		...
		if (randint(3) == 1) {
			...
			msg_print("You identified that trap as Bee Trap.");
		}
		break;

	case TRAP_OF_TERRAIN_BATTLEMENT:
		ident = TRUE;
		fill_area_terrain(p_ptr->py, p_ptr->px, 4, FEAT_BATTLEMENT, 10);

		msg_print("Battlements are constructed!");
		...
		if (randint(3) == 1) {
			...
			msg_print("You identified that trap as Battlement Trap.");
		}
		break;

	case TRAP_OF_TERRAIN_COLUMN:
		ident = TRUE;
		fill_area_terrain(p_ptr->py, p_ptr->px, 4, FEAT_COLUMN, 10);

		msg_print("Colums shoot out of the ground!");
		...
		if (randint(3) == 1) {
			...
			msg_print("You identified that trap as Column Trap.");
		}
		break;

	case TRAP_OF_TERRAIN_CRACKEDGLASS:
		ident = TRUE;
		fill_area_terrain(p_ptr->py, p_ptr->px, 4, FEAT_CRACKED_GLASS, 10);

		msg_print("Glass walls spring up out of nowhere!");
		...
		if (randint(3) == 1) {
			...
			msg_print("You identified that trap as Cracked Glass Trap.");
		}
		break;
...
void multitraptrigger(int triggercnt)
{
	cave_type *c_ptr;
	...
	while (triggercnt > 0) {
		...
		/* Try to make a new trap underneath the player */
		project(0, 0, p_ptr->py, p_ptr->px, 0, GF_MAKE_TRAP, flg);

		if (c_ptr->t_idx != 0)
		{
			/* reveal the trap if it's a type that normally becomes revealed upon triggering */
...
{
	/* switcher cures nasty traps only if their level isn't higher than the current dungeon level --Amy
	 * this is so that you won't just reroll dungeon level 1 over and over and over again until you finally find a switcher */
	int effect_level = 0;
	if (dun_level > 0) effect_level = dun_level;

	if (p_ptr->nastytrap127) { /* switchback trap: gets cured, but prevents the switcher from curing other nasty trap effects --Amy */
		if (effect_level >= 25) p_ptr->nastytrap127 = FALSE;
		calc_bonuses(TRUE);
		return;
	}

	if (effect_level >= 1) p_ptr->nastytrap1 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap2 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap3 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap4 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap5 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap6 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap7 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap8 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap9 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap10 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap11 = FALSE;
	if (effect_level >= 80) p_ptr->nastytrap12 = FALSE;
	if (effect_level >= 100) p_ptr->nastytrap13 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap14 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap15 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap16 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap17 = FALSE;
	if (effect_level >= 80) p_ptr->nastytrap18 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap19 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap20 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap21 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap22 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap23 = FALSE;
	if (effect_level >= 13) p_ptr->nastytrap24 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap25 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap26 = FALSE;
	if (effect_level >= 16) p_ptr->nastytrap27 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap28 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap29 = FALSE;
	if (effect_level >= 8) p_ptr->nastytrap30 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap31 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap32 = FALSE;
	if (effect_level >= 22) p_ptr->nastytrap33 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap34 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap35 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap36 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap37 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap38 = FALSE;
	if (effect_level >= 33) p_ptr->nastytrap39 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap40 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap41 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap42 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap43 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap44 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap45 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap46 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap47 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap48 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap49 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap50 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap51 = FALSE;
	if (effect_level >= 35) p_ptr->nastytrap52 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap53 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap54 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap55 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap56 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap57 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap58 = FALSE;
	if (effect_level >= 60) p_ptr->nastytrap59 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap60 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap61 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap62 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap63 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap64 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap65 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap66 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap67 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap68 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap69 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap70 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap71 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap72 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap73 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap74 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap75 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap76 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap77 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap78 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap79 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap80 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap81 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap82 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap83 = FALSE;
	if (effect_level >= 24) p_ptr->nastytrap84 = FALSE;
	if (effect_level >= 33) p_ptr->nastytrap85 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap86 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap87 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap88 = FALSE;
	if (effect_level >= 12) p_ptr->nastytrap89 = FALSE;
	if (effect_level >= 8) p_ptr->nastytrap90 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap91 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap92 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap93 = FALSE;
	if (effect_level >= 8) p_ptr->nastytrap94 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap95 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap96 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap97 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap98 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap99 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap100 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap101 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap102 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap103 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap104 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap105 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap106 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap107 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap108 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap109 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap110 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap111 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap112 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap113 = FALSE;
	if (effect_level >= 2) p_ptr->nastytrap114 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap115 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap116 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap117 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap118 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap119 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap120 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap121 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap122 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap123 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap124 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap125 = FALSE;
	if (effect_level >= 12) p_ptr->nastytrap126 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap127 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap128 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap129 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap130 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap131 = FALSE;
	if (effect_level >= 24) p_ptr->nastytrap132 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap133 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap134 = FALSE;
	if (effect_level >= 2) p_ptr->nastytrap135 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap136 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap137 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap138 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap139 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap140 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap141 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap142 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap143 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap144 = FALSE;
	if (effect_level >= 3) p_ptr->nastytrap145 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap146 = FALSE;
	if (effect_level >= 2) p_ptr->nastytrap147 = FALSE;
	if (effect_level >= 2) p_ptr->nastytrap148 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap149 = FALSE;
	if (effect_level >= 50) p_ptr->nastytrap150 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap151 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap152 = FALSE;
	if (effect_level >= 60) p_ptr->nastytrap153 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap154 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap155 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap156 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap157 = FALSE;
	if (effect_level >= 12) p_ptr->nastytrap158 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap159 = FALSE;
	if (effect_level >= 25) p_ptr->nastytrap160 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap161 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap162 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap163 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap164 = FALSE;
	if (effect_level >= 40) p_ptr->nastytrap165 = FALSE;
	if (effect_level >= 15) p_ptr->nastytrap166 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap167 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap168 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap169 = FALSE;
	if (effect_level >= 33) p_ptr->nastytrap170 = FALSE;
	if (effect_level >= 3) p_ptr->nastytrap171 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap172 = FALSE;
	if (effect_level >= 12) p_ptr->nastytrap173 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap174 = FALSE;
	if (effect_level >= 10) p_ptr->nastytrap175 = FALSE;
	...
	if (effect_level >= 25) p_ptr->nastytrap188 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap189 = FALSE;
	if (effect_level >= 30) p_ptr->nastytrap190 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap191 = FALSE;
	if (effect_level >= 35) p_ptr->nastytrap192 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap193 = FALSE;
	if (effect_level >= 1) p_ptr->nastytrap194 = FALSE;
	if (effect_level >= 20) p_ptr->nastytrap195 = FALSE;
	if (effect_level >= 5) p_ptr->nastytrap196 = FALSE;
	...
	if (effect_level >= 1) p_ptr->nastytrap206 = FALSE;

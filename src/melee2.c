		...
		case 128 + 28:
			{
				...
				if (p_ptr->resist_blind && !p_ptr->nastytrap29 && (rand_int(100) > 4) )
				{
		...
		case 128 + 29:
			{
				...
				if (p_ptr->resist_conf && !p_ptr->nastytrap28 && (rand_int(100) > 4) )
				{
...
static void process_monster(int m_idx, bool is_frien)
{
	...
	if (m_ptr->stunned)
	{
		...
		{
			int unstunamount = 10 + (m_ptr->level * 50);
			if (unstunamount > 1) unstunamount = randint(unstunamount);

			/* Recover fully if the monster-level-based random number is high enough */
			if (unstunamount > m_ptr->stunned) d = m_ptr->stunned;
			else d = unstunamount;
		}
	...
	for (i = 0; mm[i]; i++)
	{
		...
		if (do_move)
		{
			...
			/* Possible disturb */
			if (m_ptr->ml && ( (disturb_move && !p_ptr->nastytrap160) ||
			                  ((m_ptr->mflag & (MFLAG_VIEW)) &&
			                   (disturb_near && !p_ptr->nastytrap160))))
			{
				/* Disturb */
				if ((is_friend(m_ptr) < 0) || (disturb_pets && !p_ptr->nastytrap160) )
					disturb(0, 0);
			}

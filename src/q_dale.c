#undef cquest
#define cquest (quest[QUEST_DALE])

bool quest_dale_gen_hook(char *fmt)
{
	int x, y, i;
	int xstart = 2;
	int ystart = 2;

	if (p_ptr->inside_quest != QUEST_DALE) return FALSE;
	...
	process_dungeon_file(NULL, "grass.map", &ystart, &xstart, cur_hgt, cur_wid, TRUE);
	...
	for (i = damroll(5, 4); i > 0; )
	{
		...
		{
			summon_specific(y, x, 5, SUMMON_GRASS);
			--i;
		}
	}
	...
	for (i = damroll(2, 2); i > 0; )
	{
		...
		if (!(flags & FF1_PERMANENT) && (flags & FF1_FLOOR))
		{
			place_monster_one(y, x, 35, 0, magik(50), MSTATUS_ENEMY);
			--i;
		}
	}
...
bool quest_dale_death_hook(char *fmt)
{
	int i, mcnt = 0;
	...
	if (mcnt <= 1)
	{
		...
		del_hook(HOOK_MONSTER_DEATH, quest_dale_death_hook);
		del_hook(HOOK_GEN_QUEST, quest_dale_gen_hook);
		process_hooks_restart = TRUE;

		p_ptr->skill_points += 1;
		cmsg_format(TERM_L_GREEN, "You can increase %d more skills.", p_ptr->skill_points);

		cmsg_print(TERM_YELLOW, "Dale is safer now.");
		return (FALSE);
	}
	return FALSE;
}

bool quest_dale_finish_hook(char *fmt)
{
	s32b q_idx;

	q_idx = get_next_arg(fmt);

	if (q_idx != QUEST_DALE) return FALSE;
...
bool quest_dale_init_hook(int q_idx)
{
	if ((cquest.status >= QUEST_STATUS_UNTAKEN) && (cquest.status < QUEST_STATUS_FINISHED))
	{
		...
		if (cquest.status < QUEST_STATUS_COMPLETED) add_hook(HOOK_MONSTER_DEATH, quest_dale_death_hook, "dale_monster_death");
		add_hook(HOOK_QUEST_FINISH, quest_dale_finish_hook, "dale_finish");
		add_hook(HOOK_GEN_QUEST, quest_dale_gen_hook, "dale_geb");
	}
	return (FALSE);
}

#undef cquest
#define cquest (quest[QUEST_HELM])

bool quest_helm_gen_hook(char *fmt)
{
	int x, y, i;
	int xstart = 2;
	int ystart = 2;

	if (p_ptr->inside_quest != QUEST_HELM) return FALSE;
	...
	process_dungeon_file(NULL, "waste.map", &ystart, &xstart, cur_hgt, cur_wid, TRUE);
	...
	for (i = damroll(5, 4); i > 0; )
	{
		...
		{
			summon_specific(y, x, 35, SUMMON_WASTE);
			--i;
		}
	}
	...
	for (i = damroll(2, 2); i > 0; )
	{
		...
		if (!(flags & FF1_PERMANENT) && (flags & FF1_FLOOR))
		{
			place_monster_one(y, x, 3827, 0, magik(50), MSTATUS_ENEMY);
			--i;
		}
	}
...
bool quest_helm_death_hook(char *fmt)
{
	int i, mcnt = 0;
	...
	if (mcnt <= 1)
	{
		...
		del_hook(HOOK_MONSTER_DEATH, quest_helm_death_hook);
		del_hook(HOOK_GEN_QUEST, quest_helm_gen_hook);
		process_hooks_restart = TRUE;

		p_ptr->skill_points += 1;
		cmsg_format(TERM_L_GREEN, "You can increase %d more skills.", p_ptr->skill_points);

		cmsg_print(TERM_YELLOW, "Helm's Deep is safer now.");
		return (FALSE);
	}
	return FALSE;
}

bool quest_helm_finish_hook(char *fmt)
{
	s32b q_idx;

	q_idx = get_next_arg(fmt);

	if (q_idx != QUEST_HELM) return FALSE;
...
bool quest_helm_init_hook(int q_idx)
{
	if ((cquest.status >= QUEST_STATUS_UNTAKEN) && (cquest.status < QUEST_STATUS_FINISHED))
	{
		...
		if (cquest.status < QUEST_STATUS_COMPLETED) add_hook(HOOK_MONSTER_DEATH, quest_helm_death_hook, "helm_monster_death");
		add_hook(HOOK_QUEST_FINISH, quest_helm_finish_hook, "helm_finish");
		add_hook(HOOK_GEN_QUEST, quest_helm_gen_hook, "helm_geb");
	}
	return (FALSE);
}

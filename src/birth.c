...
static bool player_birth_aux_ask()
{
	...
	else if (!strcmp(spp_ptr->title + c_name, "Total Ascetic") || !strcmp(spp_ptr->title + c_name, "AtheistPriest") || !strcmp(spp_ptr->title + c_name, "Unbeliever") || !strcmp(spp_ptr->title + c_name, "Cosmic Mage") || !strcmp(spp_ptr->title + c_name, "Tourist") || !strcmp(spp_ptr->title + c_name, "Polymath") )
	{
		p_ptr->pgod = GOD_NONE;
	}
	else
...
static bool player_birth_aux()
{
		...
		while (randoskills > 0) {
			...
			switch (randint(3)) {
				...
				case 3:
					...
					if ((increasetempvar < 3) && (randint(5) != 1) ) increasetempvar = 3;
					if (randint(5) == 1) increasetempvar = 9;
					else if (randint(20) == 1) {
						increasetempvar = 10;
						if (randint(10) == 1) increasetempvar + randint(10); /* jackpot! */
					}
					increasetempvar *= 100; /* 0.1-1.0 to multiplier, but rarely above 0.8 or below 0.3 */
					s_ptr->mod += increasetempvar;
					break;
			}
		}
	...
	if (PRACE_FLAGS(PR1_VERY_RANDOM_SKILLS)) {
		int randoskills = 24 + randint(11); /* 25-35 */
		int skilltoincrease = 1;
		skill_type *s_ptr;
		int increasetempvar = 0;

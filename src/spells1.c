...
void teleport_player_to(int ny, int nx)
{
	...
	if (p_ptr->nastytrap15)
	{
		msg_print("Somehow, you don't get teleported!");
		return;
	}
...
void teleport_player_level(void)
{
	if (p_ptr->nastytrap15)
	{
		msg_print("Somehow, the level teleportation doesn't work!");
		return;
	}
...
void teleport_player_level_trap(void)
{
	if (p_ptr->nastytrap15)
	{
		msg_print("Somehow, the trap doesn't affect you.");
		return;
	}
...
void recall_player(int d, int f)
{
	...
	{
		msg_print("You can't recall. Sorry.");
		return;
	}
...
void acid_dam(int dam, cptr kb_str)
{
	...
	/* Inventory damage */
	if (!(p_ptr->oppose_acid && p_ptr->resist_acid) || p_ptr->nastytrap33)
...
void elec_dam(int dam, cptr kb_str)
{
	...
	/* Inventory damage */
	if (!(p_ptr->oppose_elec && p_ptr->resist_elec) || p_ptr->nastytrap33)
...
void fire_dam(int dam, cptr kb_str)
{
	...
	/* Inventory damage */
	if (!(p_ptr->resist_fire && p_ptr->oppose_fire) || p_ptr->nastytrap33)
...
void cold_dam(int dam, cptr kb_str)
{
	...
	/* Inventory damage */
	if (!(p_ptr->resist_cold && p_ptr->oppose_cold) || p_ptr->nastytrap33)
...
bool project_m(int who, int r, int y, int x, int dam, int typ)
{
	...
	case GF_SHARDS:
		{
			...
			if ((r_ptr->flags4 & (RF4_BR_SHAR)) || (r_ptr->flags7 & (RF7_RES_SHAR)) )
			{
				note = " resists.";
				dam *= 3;
				dam /= (randint(6) + 6);
	...
	case GF_FORCE:
		{
			 ...
			 */
			if ((who == 0) && !(r_ptr->flags7 & (RF7_RES_WALL)) && !(r_ptr->flags4 & (RF4_BR_WALL)) && magik(75))
			{
				a = 0;
				b = 0;
			...
			else if ((who != 0) && !(r_ptr->flags7 & (RF7_RES_WALL)) && !(r_ptr->flags4 & (RF4_BR_WALL)) ) do_stun = (randint(15) + r) / (r + 1);

			if ((r_ptr->flags4 & (RF4_BR_WALL)) || (r_ptr->flags7 & (RF7_RES_WALL)) )
			{
				note = " resists.";
				dam *= 3;
				dam /= (randint(6) + 6);
	...
	case GF_INERTIA:
		{
			if (seen) obvious = TRUE;
			if ((r_ptr->flags4 & (RF4_BR_INER)) || (r_ptr->flags7 & (RF7_RES_INER)) )
			{
				note = " resists.";
				dam *= 3;
				dam /= (randint(6) + 6);
	...
	case GF_GRAVITY:
		{
			...
			if ((r_ptr->flags4 & (RF4_BR_GRAV)) || (r_ptr->flags7 & (RF7_RES_GRAV)) )
			{
				note = " resists.";
				dam *= 3;
				dam /= (randint(6) + 6);
...
static bool project_p(int who, int r, int y, int x, int dam, int typ, int a_rad, int canreflect)
{
	...
	if ((!a_rad) && canreflect && get_skill(SKILL_DODGE) && (who > 0))
	{
	...
	case GF_SHARDS:
		{
			...
			if ((!p_ptr->resist_shard) || p_ptr->nastytrap33 || (randint(13) == 1))
			{
				if (!p_ptr->resist_shard || p_ptr->nastytrap33 || !p_ptr->plr_invprot) inven_damage(set_cold_destroy, 2);
			}
	...
	case GF_CONFUSION:
		{
			...
			if (p_ptr->resist_conf && !p_ptr->nastytrap28)
			{
				dam *= 5;
				dam /= (randint(6) + 6);
			}
	...
	case GF_ROCKET:
		{
			...
			if ((!p_ptr->resist_shard) || p_ptr->nastytrap33 || (randint(12) == 1))
			{
				if (!p_ptr->resist_shard || p_ptr->nastytrap33 || !p_ptr->plr_invprot) inven_damage(set_cold_destroy, 3);
			}
	...
	case GF_LITE:
		{
			...
			else if (!blind && (!p_ptr->resist_blind || p_ptr->nastytrap29 || (rand_int(100) < 5) ) )
			{
	...
	case GF_GRAVITY:
		{
			...
			if (!unsafe)
			{
				...
				if ((!p_ptr->ffall) || p_ptr->nastytrap33 || (randint(13) == 1))
				{
					if (!p_ptr->ffall || p_ptr->nastytrap33 || !p_ptr->plr_invprot) inven_damage(set_cold_destroy, 2);
				}
	...
	case GF_METEOR:
		{
			...
			if ((!p_ptr->resist_shard) || p_ptr->nastytrap33 || (randint(13) == 1))
			{
				if (!p_ptr->immune_fire || p_ptr->nastytrap33) inven_damage(set_fire_destroy, 2);
				if (!p_ptr->resist_shard || p_ptr->nastytrap33 || !p_ptr->plr_invprot) inven_damage(set_cold_destroy, 2);
			}
	...
	case GF_ICE:
		{
			...
			if ((!(p_ptr->resist_cold || p_ptr->nastytrap33 || p_ptr->oppose_cold)) || (randint(12) == 1))
			{
				if (!(p_ptr->immune_cold) || p_ptr->nastytrap33) inven_damage(set_cold_destroy, 3);
			}
...
bool project(int who, int rad, int y, int x, int dam, int typ, int flg)
{
	...
	if ((who == -101) || (who == -100))
		...
		if (p_ptr->nastytrap187) { /* low range nastytrap limits them to 10 squares for the player's spells --Amy */
			path_n = project_path(path_g, (who <= 0) ? MAX_RANGE_NASTY : p_ptr->nastytrap210 ? MAX_RANGE_MONST_ELONG : MAX_RANGE_MONST, y1, x1, y2, x2, flg);
		} else {
			path_n = project_path(path_g, (who <= 0) ? MAX_RANGE : p_ptr->nastytrap210 ? MAX_RANGE_MONST_ELONG : MAX_RANGE_MONST, y1, x1, y2, x2, flg);
		}
	}

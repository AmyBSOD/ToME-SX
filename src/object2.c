...
s32b flag_cost(object_type * o_ptr, int plusses, bool isshop)
{
	...
	if (f6 & TR6_RES_NERVE) total += 2000;
	if (f6 & TR6_RES_MIND) total += 2000;
	...
	case TV_AMMO_ASSAULT:
		{
			if ((power == 1) && !o_ptr->name2)
			{
				...
				{
					/* Exploding missile */
					int power[32] = {GF_ELEC, GF_POIS, GF_ACID,
							     ...
							     GF_NERVE, GF_MIND, GF_ETHER,
					                 GF_RADIOWAVE, GF_AMOEBAE, GF_DISINTEGRATE};

					o_ptr->pval2 = power[rand_int(32)];
				}
			}
			break;
		}
	...
	case TV_AMULET:
		{
			...
			case SV_AMULET_RESISTANCE:
				{
					if (randint(3) == 1) random_resistance(o_ptr, FALSE, ((randint(36)) + 4));
